...
def parse_metadata(metadata_raw: dict) -> dict:
    ...
    metadata = {
        ...
        'cartridge_overall_length_in': float(metadata_raw['Cartridge Overall Length (in)']),
...
def metadata_to_config(metadata: dict, db_path: str | None = None) -> BallisticsConfig:
    ...
    config = BallisticsConfig(
        ...
        charge_mass_gr=metadata.get('charge_mass_gr', 40.0),  # Will be overridden in fitting
        ...
        p_initial_psi=metadata.get('p_initial_psi')  # Uses bullet.p_initial_psi if None
    )

    return config


def export_fit_results(fit_result: dict, output_path: str, format: str = 'json',
                       propellant_name: str | None = None) -> None:
...
def _extract_grt_measurements(root_elem) -> pd.DataFrame:
    ...
    if measurement_elem is None:
        return pd.DataFrame(columns=['charge_grains', 'mean_velocity_fps', 'velocity_sd', 'notes'])
    ...
    for charge in measurement_elem.findall('.//charge'):
        ...
        if len(velocities_fps) > 1:
            variance = sum((v - mean_velocity)**2 for v in velocities_fps) / (len(velocities_fps) - 1)
