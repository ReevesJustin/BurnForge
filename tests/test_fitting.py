...
def test_fit_convergence():
    ...
    # Fit
    fit_result = fit_vivacity_polynomial(load_data, config_base, verbose=False)
...
def test_bounds_enforcement():
    ...
    # Check bounds
    assert 30.0 <= fit_result["Lambda_base"] <= 100.0, (
        "Lambda_base violates custom bounds"
    )
    for coeff in fit_result["coeffs"]:
...
def test_regularization():
    ...
    fit_no_reg = fit_vivacity_polynomial(
        load_data, config_base, regularization=0.0, verbose=False
    )

    # Fit with regularization
    fit_with_reg = fit_vivacity_polynomial(
        load_data, config_base, regularization=0.01, verbose=False
    )
...
def test_insufficient_data():
    ...
    # Only 2 data points
    load_data = pd.DataFrame(
        {"charge_grains": [40.0, 41.0], "mean_velocity_fps": [2550.0, 2600.0]}
    )
...
def test_missing_columns():
    ...
    load_data = pd.DataFrame({"charge_grains": [40.0, 41.0, 42.0]})
