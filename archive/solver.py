...
def load_properties_from_db(
    powder_type, bullet_type, db_path="data/db/ballistics_data.db"
):
...
def internal_ballistics_solver(
    M_gr,
    C_gr,
    D,
    V_C,
    ...
    powder_type,
    bullet_type,
    propellant_temp_F=70,
    Phi=0.9,
    P_IN=5000,
    calibrate_mv=None,
    Lambda_base_override=None,
    poly_coeffs_override=None,
    verbose=True,
):
    ...
    (
        ...
        density,
        poly_coeffs,
    ) = load_properties_from_db(powder_type, bullet_type)
    Lambda_initial = (
        Lambda_base_override if Lambda_base_override is not None else Lambda_base
    )
    poly_coeffs_used = (
        poly_coeffs_override if poly_coeffs_override is not None else poly_coeffs
    )
    ...
    def run_solver(Lambda):
        ...
        while distance < L_eff and iteration < max_iterations:
            ...
            if Z >= 1.0 and P_const is not None:
                ...
                pressure = (
                    max(0, (C * Z * F - energy_loss) / volume) if volume > 0 else 0
                )
        ...
        if iteration >= max_iterations and verbose:
            print(
                f"!WARNING! Max iterations reached. Final distance: {distance:.3f} in vs. {L_eff:.3f} in"
            )
        ...
        if peak_pressure > PRESSURE_THRESHOLD and verbose:
            print(
                f"!WARNING! Peak pressure {float(peak_pressure):.0f} PSI exceeds {PRESSURE_THRESHOLD} PSI"
            )
        ...
        return (
            ...
            peak_pressure,
            Z,
            distance_burnout if distance_burnout is not None else float("inf"),
            time,
            distance_max_pressure,
            muzzle_pressure,
        )
    ...
    results = {
        ...
        "propellant_temp_F": propellant_temp_F,
        "vivacity_base": Lambda_base * 1450,
        "vivacity": Lambda * 1450,
        "muzzle_velocity_fps": mv,
        "muzzle_pressure_psi": muzzle_p,
        "peak_pressure_psi": peak_p,
        "distance_max_pressure_in": d_max_p,
        "distance_burnout_in": burnout_dist if burnout_dist < L_eff else "Not reached",
        "final_burn_fraction": Z,
        "total_time_s": time,
    }
...
if __name__ == "__main__":
    default_inputs = {
        ...
        "D": 0.308,
        "V_C": 47.4,
        "barrel_length": 16.625,
        "case_length": 2.010,
        "powder_type": "N140",
        "bullet_type": "Copper Jacket over Lead",
        "propellant_temp_F": 36,
    }
    if len(sys.argv) > 1:
        try:
    ...
    results = internal_ballistics_solver(
        ...
        inputs["D"],
        inputs["V_C"],
        inputs["barrel_length"],
        inputs["case_length"],
        inputs["powder_type"],
        inputs["bullet_type"],
        inputs["propellant_temp_F"],
        calibrate_mv=calibrate_mv,
    )
