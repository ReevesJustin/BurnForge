...
if abs(np.mean(second_half) - np.mean(first_half)) > 5:
    ...
    print(
        "     Model consistently over-predicts at low charges and under-predicts at high charges"
    )
else:
...
print(
    f"  Improvement from polynomial: {best_rmse_constant - fit_result['rmse_velocity']:.2f} fps"
)

if best_rmse_constant - fit_result["rmse_velocity"] < 1.0:
    print(
        "  ℹ️  Polynomial provides minimal improvement - model may be limited by physics"
    )
...
print(f"\n1. Fit Quality:")
print(
    f"   - RMSE: {fit_result['rmse_velocity']:.2f} fps ({fit_result['rmse_velocity'] / np.mean(measured) * 100:.2f}% of mean velocity)"
)
...
if abs(np.mean(second_half) - np.mean(first_half)) > 5:
    ...
    print(
        f"     a) Model physics (heat loss, effective mass) not fully capturing behavior"
    )
...
print(
    f"   - Fitted value {Lambda_base / config.propellant.Lambda_base:.1f}x higher ({Lambda_base:.6f})"
)
...
if best_rmse_constant - fit_result["rmse_velocity"] > 1.0:
    print(
        f"   - Provides {best_rmse_constant - fit_result['rmse_velocity']:.1f} fps RMSE improvement over constant"
    )
