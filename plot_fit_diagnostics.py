...
fig.suptitle(
    'Fitting Diagnostics: 6.5 Creedmoor, 130gr, N150, 18" Barrel, 87°F',
    fontsize=14,
    fontweight="bold",
)
...
Lambda_vals = [
    calc_vivacity(Z, fit_result["Lambda_base"], fit_result["coeffs"]) for Z in Z_vals
]
ax3.plot(Z_vals, Lambda_vals, "b-", linewidth=2.5, label="Fitted Λ(Z)")
ax3.axhline(
    y=fit_result["Lambda_base"],
    color="r",
    linestyle="--",
    linewidth=2,
    label=f"Lambda_base = {fit_result['Lambda_base']:.6f}",
)
ax3.axhline(
    y=config.propellant.Lambda_base,
    color="g",
    linestyle=":",
    linewidth=2,
    label=f"Database = {config.propellant.Lambda_base:.6f}",
)
...
ax5.set_title(
    "Cumulative Residual (Shows Systematic Bias)", fontsize=12, fontweight="bold"
)
...
# Shade regions
ax6.axhspan(
    1.0, max(poly_multipliers), alpha=0.2, color="green", label="Increased burn rate"
)
